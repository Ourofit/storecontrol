...
function Expenses(props) {
	...
	useEffect(() => {
		...
		async function store_expense() {
			if(Status && window.desktop) {// I think I forgot to remove it. It is in allFunction file i think. But I left it in here for saving time because We are late to submit this project
				await window.api.getAllData("Expenses").then(async (item2) => {
					await axios.get("http://localhost:5000/expense").then(async (item) => {
