							...
							item.data.forEach(async function(ex) {
								...
								if(flag === 0) {
									...
									await axios.delete(`http://localhost:5000/expense/delete/${ex.ExpenseId}`).then(async dele => {
										await axios.get("http://localhost:5000/expense").then(async (item7) => {// To update the Redux state so that the client does not have refresh the page everytime the 
