...
function Expenses(props) {
	...
	useEffect(() => {
		...
		async function exp() {
            await store_Expenses('Expenses', Status, Expenses, allexp)//why did you do it . i know that here call the same function
		...
		async function store_expense() {
			if(Status && window.desktop) {// I think I forgot to remove it. It is in allFunction file i think. But I left it in here for saving time because We are late to submit this project. dont worry it is not causing any problem I think.
				//okey and I have other doumbt
				await window.api.getAllData("Expenses").then(async (item2) => {
					await axios.get("http://localhost:5000/expense").then(async (item) => {
							...
							item.data.forEach(async function(ex) {
								...
								if(flag === 0) {
									...
									await axios.delete(`http://localhost:5000/expense/delete/${ex.ExpenseId}`).then(async dele => {
										await axios.get("http://localhost:5000/expense").then(async (item7) => {// To update the Redux state so that the client does not have refresh the page everytime they insert or delete the record okey awesome 
