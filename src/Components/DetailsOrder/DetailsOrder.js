...
function DetailsOrder({ details_data, setDetailsData, order, setOrder, product, setReturnVal=null, particularOrder = null, ...props }) {
	...
	// 			await axios.delete(`https://storecontrolserver-production-3675.up.railway.app/ordermaster/delete/${order.Order_id}`)
	// 			await axios.delete(`https://storecontrolserver-production-3675.up.railway.app/orderproduct/delete/${val.Order_pro_id}`)
	// 			.then(async item => {
	...
	// 		if(Status) {
	// 			await axios.put('https://storecontrolserver-production-3675.up.railway.app/product/quantity', req_data_el)
												...
												<div className='order_status my-1'>
													...
													{order?.Order_status === 'Paid' ? (
														<button 
															...
															onClick={async () => {
																await axios.put('https://storecontrolserver-production-3675.up.railway.app/ordermaster/status', {
																	Order_id: order.Order_id,
																	Order_status: 'Unpaid'
																})
																await axios.get('https://storecontrolserver-production-3675.up.railway.app/ordermaster')
