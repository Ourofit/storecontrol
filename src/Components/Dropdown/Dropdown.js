...
function Dropdown({ name, dropvalues, onChange, touched, errors, value_select, inputbox=false, ...props }) {
    ...
    const input_submit = async (e, name) => {
        ...
        if(name === 'Category_id') {
            if(Status) {
                await axios.post('https://storecontrolserver-production-3675.up.railway.app/category/new', { Category_id: uuidv4(), nombre: inputText})
                    .then(async (item) => {
                        category(item.data)
            ...
            if(deposit.length >= 2 ) {
                ...
                if(de.includes(inputText)) {
                    ...
                    de.push(inputText)
                    await axios.put('https://storecontrolserver-production-3675.up.railway.app/deposito/employee', { Deposito_id: deposit.Deposito_id,  Employee_list: JSON.stringify(de)})
                        .then(async (item) => {
                            // console.log(de)
                            var dep = {
                            ...
                            DepositoLog(dep)
                            await axios.get('https://storecontrolserver-production-3675.up.railway.app/deposito')
                                .then(item => {
                                    deposito(item.data)
                                    setInputText('')
                                })
            ...
            if(Status) {
                await axios.post('https://storecontrolserver-production-3675.up.railway.app/expensecat/new', {nombre: inputText}).then(async (item)=>{
        ...
        //     await axios.post('https://storecontrolserver-production-3675.up.railway.app/deposito/new', { nombre: inputText})
        //         .then((item) => {
        //             deposito(item.data)
        //             setInputText('')
        //         })
    ...
    const category_remove = async (cate, index) => {
        ...
        category(filtered)
        if(Status) {
            await axios.delete(`https://storecontrolserver-production-3675.up.railway.app/category/delete/${CategoryAdd.filter(function(el, i) { return index === i; })[0].Category_id}`)
        } else {
            if(window.desktop) {
