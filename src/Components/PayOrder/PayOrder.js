...
function PayOrder({ Province, deposit, details_data, setDetailsData, order, setOrder, setOrderReturn, setOrder_Data, returnProduct, return_val, setReturnedData = null, setOrderDetails = null, returned_data = null, ...props }) {
            ...
            if (client_name === '') {
                ...
                if (Status) {
                    if (Orders.find(ele => ele.Order_id === o.Order_id) !== undefined) {
                        ...
                        await axios.put('https://storecontrolserver-production-3675.up.railway.app/ordermaster/update', o)
                            .then(async (item) => {
                                var data_ord = []
                                ...
                                await axios.post('https://storecontrolserver-production-3675.up.railway.app/orderproduct/new', data_ord)
                                    .then(async (item2) => {
                                        ...
                                        for (let i = 0; i < data_ord.length; i++) {
                                            ...
                                            await axios.put('https://storecontrolserver-production-3675.up.railway.app/product/quantity', req_data)
                        ...
                        await axios.post('https://storecontrolserver-production-3675.up.railway.app/ordermaster/new', o)
                            .then(async (item) => {
                                for (let i = 0; i < details_data.length; i++) {
                                    details_data[i].Order_id = item.data.Order_id
                                }
                                await axios.post('https://storecontrolserver-production-3675.up.railway.app/orderproduct/new', details_data)
                                    .then(async (item2) => {
                                        for (let i = 0; i < details_data.length; i++) {
    ...
    return (
        <div className='payorder'>
            ...
            <NewClient  Province={Province} depositVal={deposit} />
        </div>
    )
}
