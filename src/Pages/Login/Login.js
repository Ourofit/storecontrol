...
function Login({...props}) {
							...
							await window.api.getAllData('Deposito').then(async (dep_item) => {
								dep_item.Deposito.forEach(async function(dep) {
									if(!Object.keys(dep).includes('Deposito_id')) {
										...
										await axios.post('https://storecontrolserver-production-3675.up.railway.app/deposito/new', dep).then(async (item_new) => {
	...
	const onSubmit = async (values, { resetForm }) => {
		...
		if(btn_name === 'Login') {
			...
			if(Status) {
				await axios.post('https://storecontrolserver-production-3675.up.railway.app/deposito/new', dep)
					.then(async (item) => {
